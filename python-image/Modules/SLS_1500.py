...
def process_sls_1500(dict_parameters):
    print("SLS_1500 processing started")
    exp_folder = dict_parameters['exp_name']
    flow_subfolder = dict_parameters['flow_subfolder']
    total_seconds = dict_parameters['total_seconds']
    file_name = exp_folder+'/'+flow_subfolder
    sls_interval_string = dict_parameters['sls_interval']
    sls_interval = bytes(sls_interval_string, 'utf-8')

    for subfolder_path in [exp_folder+'/'+flow_subfolder]:
...
class SLS_1500Device(ShdlcDeviceBase):
    ...
    def Continuous_Measure_and_Save(self, duration_s, filename, sls_interval, set_flow_rate_string=None):
        ...
        while i < retrievals:
            print("Retrieval %d" % i)
            self.Start_Continuous_Measurement(sls_interval)
